...
class _EditProfileState extends State<EditProfile> {
  ...
  DropdownButton<String> getLanguage(String lang) {
    ...
    for (String lang in languageList) {
      var item = DropdownMenuItem(
        value: lang,
        child: Text(lang),
      );
                  ...
                  ButtonGlobal(
                    ...
                    onPressed: () async {
                      try {
                        ...
                        // Fallback to existing values if not changed
                        final currentProfile = userProfileDetails.value!;
                        final updatedProfile = PersonalInformationModel(
                          businessCategory: dropdownValue,
                          companyName: companyName == 'nodata' ? currentProfile.companyName : companyName,
                          phoneNumber: phoneNumber == 'nodata' ? currentProfile.phoneNumber : phoneNumber,
                          countryName: initialCountry,
                          invoiceCounter: currentProfile.invoiceCounter,
                          language: dropdownLangValue,
                          pictureUrl: imagePath == 'No Data' ? currentProfile.pictureUrl : profilePicture,
                          remainingShopBalance: currentProfile.remainingShopBalance,
                          shopOpeningBalance: currentProfile.shopOpeningBalance,
                        );
                        ...
                        await _personalInformationRef.set(updatedProfile.toJson());

                        // Force refresh the profile data and wait for it to complete
                        await ref.refresh(profileDetailsProvider.future);
